...
const MainLayout = () => {
  ...
  // If viewing another user's profile (not from tab)
      ...
      <div className="min-h-screen bg-background flex flex-col">
        <header className="flex items-center justify-between px-4 py-3 border-b border-border bg-background sticky top-0 z-10">
          <div className="flex items-center gap-2">
            <img src={logo} alt="TrunkShare" className="h-10" />
        ...
        <main className="flex-1 overflow-y-auto pb-20">
          <UserProfile userId={profileUserId} onBack={() => setProfileUserId(null)} />
        </main>
        <nav className="fixed bottom-0 left-0 right-0 bg-background border-t border-border flex z-20">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => { setProfileUserId(null); setActiveTab(tab.id); }}
              className={`flex-1 flex flex-col items-center py-3 gap-1 text-xs font-medium transition-colors ${
                activeTab === tab.id && !profileUserId ? 'text-primary' : 'text-muted-foreground'
              }`}
            >
              <tab.icon size={20} />
              {tab.label}
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 overflow-y-auto pb-20">
        ...
        {activeTab === 'profile' && <UserProfile userId={user!.id} onBack={() => setActiveTab('send')} />}
      </main>
